globalThis.DBlockerMainEngine = function DBlockerMainEngine(bootstrap) {
  ...
  function overlayActivationCaptureHandler(event) {
    ...
    if (now < overlayPendingActivationUntil) {
      ...
      // actually clicks. Preserve explicit close/dismiss controls: the persistent
      // telemetry still arms browser-level quarantine, but the page gets the event
      // it needs to remove its banner. Ordinary activations keep the one-shot shield.
      emitPersistentOverlayActivation(event);
      if (event?.isTrusted === true && isLikelyDismissControl(event.target)) {
        overlayPendingActivationUntil = 0;
        return;
      }
      overlayPendingActivationUntil = 0;
    ...
    if (now < overlaySuppressionUntil) {
      ...
      // overlay was hidden during pointerdown/touchstart. Never consume an explicit
      // trusted close/dismiss action; background quarantine remains armed.
      if (event?.isTrusted === true && isLikelyDismissControl(event.target)) {
        emitPersistentOverlayActivation(event);
        return;
      }
