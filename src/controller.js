(() => {
  ...
  async function shouldBootstrapProtectedContext() {
    ...
    // contact the service worker with browsing context at all.
    const stored = await chrome.storage.local.get(S.STORAGE_KEYS.enabledSites);
    ...
    // Authoritative matching stays in the service worker because it loads the
  ...
  async function handleTelemetryDetail(detail) {
    ...
    if (verified.type === 'blocked') {
      ...
      if (!item) return;
      chrome.runtime.sendMessage({ type: 'ADS_BLOCK_EVENT', item }).catch(() => {});
      return;
    }
