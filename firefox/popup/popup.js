(() => {
  ...
  async function resolveTargetTab() {
    // Android can open this panel in a tab from an in-page management button.
    // Keep actions bound to the originating website, never to the panel itself.
    const source = new URL(location.href).searchParams.get('tabId');
    if (source !== null) {
      if (!/^\d+$/.test(source)) return null;
      try { return await browser.tabs.get(Number(source)); } catch (_) { return null; }
    }
  ...
  async function refresh() {
    browserTab = await resolveTargetTab();
    ...
    if (browserTab?.id != null) {
      const response = await browser.runtime.sendMessage({ type: 'ADS_GET_TAB_STATE', tabId: browserTab.id });
      tabState = response?.state || { items: {}, page: null };
      if (!pendingFocusLoaded) {
        pendingFocusLoaded = true;
        const focusResponse = await browser.runtime.sendMessage({ type: 'ADS_GET_PENDING_FOCUS', tabId: browserTab.id });
  ...
  function makeBlockedRow(item) {
    ...
    if (item.type === 'iframe' && !isVideoPlayer) {
      ...
      loadOnceBtn.addEventListener('click', async () => {
        ...
        await browser.runtime.sendMessage({ type: 'ADS_ALLOW_ONCE', tabId: browserTab.id, origin: item.origin });
  ...
  els.clearBtn.addEventListener('click', async () => {
    if (browserTab?.id == null) return;
    await browser.runtime.sendMessage({ type: 'ADS_CLEAR_TAB_STATE', tabId: browserTab.id });
    tabState = { items: {}, page: null };
  ...
  els.reloadTabBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // snapshot captured when the popup first opened. This also gives us a
      // Promise we can await so Firefox reload errors are not silently lost.
  ...
  if (els.supportBtn) {
    els.supportBtn.addEventListener('click', () => {
      const url = browser.runtime.getURL('options/options.html#support');
  ...
  browser.storage.onChanged.addListener(() => refresh().catch(() => {}));
